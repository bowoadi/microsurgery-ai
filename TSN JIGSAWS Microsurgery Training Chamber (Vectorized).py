...
for task in classes.keys():
    for model_type in list(models.keys())[1:2]:
        # for task in task_manual:
        classes_dict = classes[task]
        main_class = f"{task}"
        ...
        root_path = f"./jigsaws_frameskip_{frame_skip}_{video_sec}s_{frame_size}size"

        metadata_dir = "./jigsaws_model_training_data"
        model_dir = "./jigsaws_models"
        frame = f"frame_{frame_count}_skip_{frame_skip}_size_{frame_size}"
        sub_class = f"exp-{'tsn-' if tsn else ''}{model_type}"
        if fuzzy:
            sub_class += "-fuzzy"
        if augmented:
            sub_class += "-augmented"
        ...
        print(main_class, sub_class, frame)
        print(device)
        ...
        def visualize_video_frames(x, rows, cols):
            ...
            frame_step = 180 // (rows * cols)  # Determine the frame step to evenly distribute frames
            print("Skipped every: ", frame_step, "frames")
            
            # Generate the indices of frames to be sampled with the frame step
            sample_indices = np.arange(0, 180, frame_step)[:rows * cols]
        ...
        # visualize_video_frames(test_tensor, 3, 3)

        # %%
        resnet_shortcut = 'B'
        last_fc = True
        torch.cuda.empty_cache()
        model = models[model_type](num_classes=n_classes, fuzzy=fuzzy, shortcut_type=resnet_shortcut,
